...
import { MIN_DEPOSIT_SOL, lamportsToSol, readPendingTicketZinc } from "@/lib/cwr";

const QUICK = [0.1, 0.5, 1, 5, 10];

/**
 * dZINC park card. Shown while the pool is cranking (BETTING), when normal
 * dZINC deposits are closed. Lets a user commit SOL now; it converts to dZINC
 * automatically when the next claim window opens. A parked ticket can be pulled
 * back any time. Mirrors the dORE ParkCard.
 */
export function ParkZincCard({ data, onDone }: { data: ZincPoolStats | null; onDone: () => void }) {
  const { connection } = useConnection();
  const { publicKey, connected } = useWallet();
  const { park, cancelPark, busy } = useZincActions();
  const [ticketSol, setTicketSol] = useState<number | null>(null);
  ...
  const refresh = useCallback(async () => {
    ...
    const t = await readPendingTicketZinc(connection, publicKey).catch(() => null);
    setTicketSol(t ? lamportsToSol(t.amountLamports) : null);
  ...
  // Park is a BETTING-phase action (phase 0). OPEN (phase 1) uses the Mint card.
  ...
  return (
    <div className="card card-hover flex flex-col">
      ...
      <p className="mb-4 font-mono text-[12px] leading-relaxed text-fog-muted">
        Deposits are closed while the pool is mining. Park SOL now and it converts to dZINC
      ...
      {ticketSol != null && (
        <div className="mb-4 rounded-lg border border-pos/40 bg-ink-800 p-3">
          <div className="flex items-center justify-between">
            ...
            <span className="num text-pos">{formatNum(ticketSol, 4)} SOL</span>
          </div>
          <p className="mt-1.5 font-mono text-[12px] text-fog-muted">
            Converts to dZINC at the next deposit/claim window. Or pull it back now.
          </p>
          <button
            disabled={busy}
            onClick={onCancel}
      ...
      {belowMin && (
        <p className="mt-2 font-mono text-[12px] text-amber">Park at least {MIN_DEPOSIT_SOL} SOL.</p>
      )}
      ...
      {connected && !cranking && (
        <p className="mt-2 text-center font-mono text-[12px] text-fog-muted">
          {data?.paused
            ...
            : !data?.initialized
              ? "Pool not live yet."
              : "The deposit/claim window is open. Use Mint dZINC to deposit directly."}
        </p>
      )}
