...
export function ZincPhaseTimers({ data }: { data: ZincPoolStats | null }) {
    ...
    return (
      <div className="card">
        <p className="font-mono text-sm text-fog-muted">
          dZINC pool isn&apos;t live on-chain yet. Timers start once bucket 1 is initialized.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="panel">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <TimerTile
          title="Deposit & Claim"
          subtitle="mint dZINC or claim rewards"
          active={clock.isOpen}
        ...
        <TimerTile
          title="Cranking"
          subtitle="capital working the 30 tiles"
      ...
      {data.ddHalt && !data.paused && (
        <p className="mt-4 font-mono text-xs text-amber">
          Drawdown halt active - mining paused until conditions recover. Deposits / claims still work.
        </p>
      )}
    </div>
  );
