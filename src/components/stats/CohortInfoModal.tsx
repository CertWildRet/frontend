...
export function CohortInfoModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return createPortal(
    ...
    >
      <div
        className="my-auto w-full max-w-xl rounded-2xl border border-line bg-ink-900 shadow-2xl"
      ...
      >
        ...
        <div className="space-y-4 px-5 py-4 font-mono text-[12.5px] leading-relaxed text-fog-dim">
              ...
              <tbody>
                {ROWS.map(([label, wallet, unclaimed], i) => (
                  <tr key={i} className="border-b border-line/60">
                    <td className="py-2 pr-3 text-fog-dim">{label}</td>
          ...
          <ul className="space-y-2 text-[11.5px] text-fog-muted">
            <li>
              <span className="text-fog-dim">Wallet Holders is liquid ORE only.</span> It counts the ORE token, not stORE. ORE that&apos;s been
              staked into stORE sits in the stake vault and shows in the separate <span className="text-fog-dim">vaulted</span> figure under the
              donut — so a pure stORE staker shows as 0 here.
            </li>
            <li>
              <span className="text-fog-dim">Unclaimed Rewards is unclaimed ORE only</span> — unclaimed mining rewards + pending refined ORE
              (both ORE, no stORE, no SOL).
            </li>
            <li>
              <span className="text-fog-dim">Vaulted is excluded from both.</span> Protocol vaults (the mine treasury + stORE stake vault, ~143k
              ORE) are reported separately, never as holder whales.
            </li>
            <li>
              <span className="text-fog-dim">A &ldquo;whale&rdquo; means different things.</span> In Wallet Holders it&apos;s a wallet holding a
