...
export default function DispersionLanding() {
  return (
    <main className="relative mx-auto w-full max-w-[1180px] px-5 pb-28 pt-8 sm:px-8">
      <SpectralDefs />
      ...
      <header className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <span
            className={`${styles.glass} grid h-10 w-10 place-items-center rounded-xl ${styles.cutTR}`}
          >
            <PrismIcon className="h-5 w-5" />
          </span>
          <span
            className="text-[19px] font-semibold tracking-[0.16em]"
            style={display}
          >
            CWR
          </span>
      ...
      <section className="relative mt-10 sm:mt-16">
        ...
        <div className="relative z-10 max-w-[760px]">
          ...
          <div className="mt-9 flex flex-wrap items-center gap-3.5">
            ...
            <a
              href="#live"
              className={`${styles.glass} ${styles.ghost} inline-flex items-center gap-2 rounded-full px-6 py-3.5 text-[15px] text-[#C6CCEC]`}
            >
              Watch it live
            </a>
          </div>
      ...
      <section className="mt-28">
        ...
        <div className="mt-9 grid gap-5 sm:grid-cols-2">
          {TEAM.map((m) => (
            ...
            >
              <span
                className={`${styles.glass} grid h-14 w-14 shrink-0 place-items-center rounded-2xl`}
              >
                <span
                  className={`${styles.spectralText} text-[18px] font-bold`}
                  style={display}
                >
      ...
      {/* ══ FOOTER ═════════════════════════════════════════════════ */}
      <footer className="mt-24 border-t border-[rgba(154,167,216,0.12)] pt-12">
        <div className="grid gap-10 md:grid-cols-[1.4fr_1fr_1fr]">
          <div>
            ...
            <p className="mt-4 max-w-[300px] text-[13.5px] leading-relaxed text-[#8A92B4]">
              Pooled, non-custodial ORE mining on Solana.
            </p>
          </div>
          <FooterCol
            title="Product"
            links={["Simple Pool", "Refined", "Live crank", "Dashboard"]}
          />
          <FooterCol title="Legal" links={["Terms", "Privacy", "Risk", "Docs"]} />
        </div>
        <div className="mt-12 flex flex-col items-center justify-between gap-4 border-t border-[rgba(154,167,216,0.1)] pt-6 sm:flex-row">
          <div className="flex items-center gap-3">
            {["X", "Discord"].map((s) => (
              <a
                key={s}
                href="#"
                className={`${styles.glass} rounded-full px-4 py-1.5 text-[12px] text-[#9AA3C8] transition-colors hover:text-[#EAECF6]`}
              >
                {s}
              </a>
            ))}
          </div>
          <span className="text-[12px] tracking-[0.2em] text-[#6E769A]" style={mono}>
            built on Solana
          </span>
        </div>
      </footer>
...
function FooterCol({ title, links }: { title: string; links: string[] }) {
  return (
    <div>
      <h4
        className="text-[12px] uppercase tracking-[0.3em] text-[#6E7AA0]"
        style={{ fontFamily: "'JetBrains Mono Variable', monospace" }}
      >
        {title}
      </h4>
      <ul className="mt-4 space-y-2.5">
        {links.map((l) => (
          <li key={l}>
            <a
              href="#"
              className="text-[14px] text-[#9AA3C8] transition-colors hover:text-[#EAECF6]"
            >
              {l}
            </a>
          </li>
